...
function AccountBalancesView() {
  ...
  const handleCheckCapitalBalance = async (accountType) => {
    setIsLoading(true);
  ...
  const handleCheckLoanBalance = async (accountType) => {
    setIsLoading(true);
    setSelectedAccount('loanIncome')
    const client = new Client('wss://s.altnet.rippletest.net:51233');
    const assetCode = 'ZAR'; // Asset code to look for
    const xrplAddress = 'rshEW2GG5BVQXsrwMqxsXnSHaihkZDR9ww'; // Hardcoded XRPL address
  ...
  const handleCheckFeeBalance = async (accountType) => {
    setIsLoading(true);
    setSelectedAccount('transactionFee')
    const client = new Client('wss://s.altnet.rippletest.net:51233');
    const assetCode = 'ZAR'; // Asset code to look for
    const xrplAddress = 'r3o4P4BNAVCR2QzCzeZXVrsvwheoh3vJa7'; // Hardcoded XRPL address
  ...
  return (
    <div className="account-balances-view-container">
      ...
      <button onClick={() => handleCheckCapitalBalance('capitalPool')}>Capital Pool Account</button>
      <button onClick={() => handleCheckLoanBalance('loanIncome')}>Loan Income Account</button>
      <button onClick={() => handleCheckFeeBalance('transactionFee')}>Transaction Fee Account</button>
